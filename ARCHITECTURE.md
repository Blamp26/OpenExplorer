...
Native ABI v4 preserves the opaque snapshot handle, two-call batched range access, and local-directory open operation, and adds an independent sorted-view handle. Interop owns native handles and provider-specific File hierarchy resolution, Application owns the provider-agnostic navigation controller, base snapshot, active sorted view, and bounded page cache, and UI consumes display rows without calling native exports. Sorting is performed in Rust and does not reopen directories. Navigation history stores locations rather than complete snapshots. No directory contents cross the boundary as JSON. Future IPC is planned as versioned Named Pipes with Protocol Buffers; neither is implemented yet.
