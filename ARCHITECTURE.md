...
Native ABI v3 exposes an opaque snapshot handle, two-call batched range access, and a local-directory open operation. Interop owns native handles and provider-specific File hierarchy resolution, Application owns the provider-agnostic navigation controller and bounded page cache, and UI consumes display rows without calling native exports. Navigation history stores locations rather than complete snapshots. No directory contents cross the boundary as JSON. Future IPC is planned as versioned Named Pipes with Protocol Buffers; neither is implemented yet.
