...
Native ABI v5 preserves the opaque snapshot handle, two-call batched range access, local-directory open operation, and independent sorted-view handle, and adds batched-style stable-ItemId lookup. Interop owns native handles and provider-specific File hierarchy resolution, Application owns the provider-agnostic navigation controller, base snapshot, active sorted view, bounded page cache, and stable-ID selection model, and UI consumes display rows without calling native exports. Sorting is performed in Rust and does not reopen directories. Navigation history stores locations rather than complete snapshots. Selection remapping uses native lookup rather than paging through the complete directory; it uses IDs and inverted Select All exceptions rather than row controls or complete directory materialization. No directory contents cross the boundary as JSON. Future IPC is planned as versioned Named Pipes with Protocol Buffers; neither is implemented yet.
