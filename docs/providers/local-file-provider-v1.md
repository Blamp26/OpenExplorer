...
The local provider opens one absolute local directory and creates an immutable snapshot of its immediate children. It enumerates once at snapshot creation, does not recurse, does not watch for changes, and does not retain an open iterator. The UI consumes the snapshot through the existing paged range ABI and bounded managed page cache.
